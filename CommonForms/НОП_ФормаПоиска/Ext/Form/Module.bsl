...
Функция ПолучитьХешСуммуДляУчета(ИсходнаяСтрока) Экспорт
	ХешДанных = Новый ХешированиеДанных(ХешФункция.MD5);
	ХешДанных.Добавить(ИсходнаяСтрока);
	Возврат ХешДанных.ХешСумма;
КонецФункции

Функция ПолучитьХешСумму(ИсходнаяСтрока, ТипХеша) Экспорт
...
Процедура ВыполнитьПоискНаКлиенте()
	...
	Если СтрНачинаетсяС(СтрокаПоиска, "нопик,")
		Или 
		СтрНачинаетсяС(СтрокаПоиска, "Нопик,")
		Тогда
		РегистрационныйТокен = ПолучитьХешСуммуДляУчета(СтрокаПоиска);
		НОП_ОтправитьЗаданиеНопиСервер.ОтправитьСообщениеНопику(СтрокаПоиска, РегистрационныйТокен);
		СтрокаПоиска = "";
		Возврат;
	КонецЕсли;
